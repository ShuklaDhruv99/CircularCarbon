...
- **Modify:**
  ...
  - `frontend/src/components/recommendations/RecommendationList.tsx` — add an "Explain Recommendations" action that calls the generate endpoint once for the whole list and matches returned explanations back to their `recommendation_id`, rendering an `ExplanationPanel` under each corresponding `RecommendationCard`
...
- `backend/app/services/explanation_service.py` — orchestration: for each recommendation of a factory (via `recommendation_service.get_for_factory`), build the prompt via `prompts.py`, call `gemini_client.generate_explanation`, parse the four labeled sections from the response text; on any `GeminiUnavailableError` or parse failure, build the same four sections from a deterministic Python string template using only the verified values (`source="fallback"`); returns `list[ExplanationRead]`
...
- [ ] When `gemini_api_key` is unset or the Gemini call fails/times out, the endpoint still returns a complete, correctly-shaped explanation for every recommendation (`source="fallback"`), verified by unit test with no real network call
